...
class TestScenarios:
    ...
    def test_find_scenario_index_by_name(self, file, model):
        ...
        assert scenarios.find_index("scenario A") == 0
        assert scenarios.find_index("scenario B") == 1
        assert scenarios.find_index("Non existing") is None
    ...
    def test_get_config_from_name(self, file, model):
        ...
        assert scenarios.config("scenario A") == scenario_a
        assert isinstance(
            scenarios.config("scenario A", as_dict=False),
            ScenarioConfig,
        )

        assert scenarios.config("scenario B") == {
            "name": "scenario B",
            "size": 2,
    ...
    def test_get_size_from_name(self, file, model):
        ...
        for name, expected_size in sizes.items():
            assert scenarios.get_size(name) == expected_size
    ...
    def test_update(self, file, model):
        ...
        assert scenarios.config("scenario B") is None
        assert scenarios.config("scenario C") == new_dict
        ...
        assert model.has_changes is True
        assert scenarios.config("scenario X") == new_dict
    ...
    def test_delete(self, file, model, scenario_name):
        ...
        assert model.scenarios.find_index(scenario_name) is None
    ...
    def test_rename(self, file, model, scenario_name):
        ...
        assert model.scenarios.exists(scenario_name) is False
        assert model.scenarios.exists(new_name) is True

        if scenario_name == "scenario A":
            assert (
                model.parameters.config("param3", as_dict=True)["scenario"] == new_name
            )
