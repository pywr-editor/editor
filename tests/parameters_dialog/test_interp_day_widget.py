...
from pywr_editor.form import InterpDayWidget, FormField
...
class TestDialogParameterInterpDayWidget:
    ...
    model_file = resolve_model_path(
        "model_dialog_parameters_interp_day_widget.json"
    )
    ...
    def test_valid_widget(self, qtbot, model_config, param_name, expected):
        ...
        interp_day_field: FormField = selected_page.findChild(
            FormField, "interp_day"
        )
        ...
        if expected is None:
            ...
            assert (
                interp_day_field.value()
                is interp_day_widget.get_default_value()
            )
        else:
            assert (
