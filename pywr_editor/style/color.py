...
ColorShade = Literal[50, 100, 200, 300, 400, 500, 600, 700, 800, 900] | str
...
class Color:
    ...
    def __init__(self, name: ColorName, shade: ColorShade):
        ...
        if self.shade not in self.colors[self.name]:
            raise KeyError(
                f"The color shade {self.shade} for the color named "
    ...
    def rgb(self) -> tuple[int]:
        ...
        if len(hex_color) == 3:  # for example #FFF
            hex_color = f"{hex_color}{hex_color}"
    ...
    def change_shade(self, new_shade: ColorShade) -> "Color":
        """
