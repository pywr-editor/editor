...
class Nodes:
    ...
    def set_position(
        ...
        """
        Set or update the editor position of a node identified by its name or index.
    ...
    def set_pywr_position(
        ...
        """
        Set or update the pywr position of a node.
        :param position: The position to set as list.
        :param node_name: The name of the node.
        :param node_index: The index of the node.
        :return: None
        ...
        if "position" in self.model.json["nodes"][node_index]:
            self.model.json["nodes"][node_index]["position"]["schematic"] = position
        else:
            self.model.json["nodes"][node_index]["position"] = {"schematic": position}
    ...
    def config(self, node_name: str, as_dict: bool = True) -> dict | NodeConfig | None:
        """
        Find the node configuration dictionary by the node name.
        :param node_name: The node to look for.
    ...
    def delete(self, node_name: str) -> None:
        """
        Delete a node and its edges from the model dictionary.
    ...
    def orphans(self) -> list[str] | None:
        ...
        for node_dict in self.get_all():
            ...
            if not node_config.is_virtual and node_config.name not in nodes_in_edges:
    ...
    def add(self, node_config: dict) -> None:
        ...
        if "name" not in node_keys or "type" not in node_keys:
            raise KeyError("The node configuration must have a name and a type set")
    ...
    def update(self, node_dict: dict) -> None:
        """
        Replace the node dictionary for an existing node. This preserves the
        node position.
        ...
        node_index = self.find_node_index_by_name(node_name)
        node_obj: NodeConfig = self.config(node_name=node_name, as_dict=False)
        if node_obj is None:
            return
        ...
        if edge_color_key in node_dict:
            new_node_dict["position"][edge_color_key] = node_dict[edge_color_key]
            del node_dict[edge_color_key]
        node_style_key = Constants.NODE_STYLE_KEY.value
        if node_style_key in node_dict:
            new_node_dict["position"][node_style_key] = node_dict[node_style_key]
    ...
    def dependencies(self, node_name: str) -> DictMatches | None:
        """
        Check the model components using the node name.
        :param node_name: The name of the node.
    ...
    def rename(self, node_name: str, new_name: str) -> None:
        """
        Rename a node. This changes the name in the "name" dictionary key
        ...
        self.model.json["edges"] = JsonUtils(self.model.json["edges"]).replace_str(
            old=node_name,
            new=new_name,
        )
