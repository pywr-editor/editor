...
class Polynomial1DParameterSection(FormSection):
    def __init__(self, form: ParameterDialogForm, section_data: dict):
        """
        Initialise the form section for a Polynomial1DParameter.
        ...
        self.add_fields(
            {
                "Configuration": [
                    FieldConfig(
                        ...
                        value={"coefficient": self.form.field_value("coefficients")},
                        help_text="Define the coefficients. You can set, as the "
                        "polynomial independent variable, the flow from a node, the "
                        "volume from a storage node or a parameter value",
                    ),
                    FieldConfig(
                        name="storage_node",
    ...
    def validate(self, form_data: dict) -> Validation:
        ...
        for field_name in ["parameter", "node", "storage_node"]:
            value = self.form.find_field(field_name).value()
